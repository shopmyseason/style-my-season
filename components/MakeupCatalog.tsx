...
type MakeupSearchResponse = {
  products: Product[];
  category: string;
  selectedPalette: SeasonalPalette | null;
};

export function MakeupCatalog() {
  ...
  const fetchProducts = useCallback(
    async (cat: string, palette: string) => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        const response = await fetch(`/api/makeup-search?${params.toString()}`);
        if (!response.ok) throw new Error("Failed to load products. Please try again.");

        const data = (await response.json()) as MakeupSearchResponse;
  ...
  const hasActiveFilters =
    !!appliedCategory || appliedPalette !== null || onlyGoodMatches;

  function handleCategoryChange(val: string) {
    setCategory(val);
    fetchProducts(val, paletteValue);
  }

  function handlePaletteChange(val: string) {
    setPaletteValue(val);
    fetchProducts(category, val);
  }

  function handleSearch(e: React.FormEvent) {
    e.preventDefault();
    fetchProducts(category, paletteValue);
  }
          ...
          <div className="rounded-2xl border border-white/80 bg-white p-5 shadow-xl shadow-gray-200/40 sm:p-6">
            ...
            >
              <div className="flex-1 space-y-1.5">
                <label
                  htmlFor="makeup-category-select"
              ...
              <div className="space-y-3 sm:w-56">
                <div className="space-y-1.5">
                  <label
                    htmlFor="makeup-palette-select"
                    className="text-xs font-semibold uppercase tracking-wider text-gray-500"
                  >
                    Palette
                  </label>
                  <select
                    id="makeup-palette-select"
                    value={paletteValue}
        ...
        <div className="mx-auto max-w-7xl">
                ...
                {isLoading
                  ...
                  : error
                    ...
                    : productsWithScores.length === 0
                      ? "No makeup products yet — check back soon!"
          ...
          {isLoading ? (
            ...
            <div className="rounded-3xl border border-rose-100 bg-white px-6 py-20 text-center shadow-sm">
              ...
              <button
                type="button"
                onClick={() => fetchProducts(category, paletteValue)}
